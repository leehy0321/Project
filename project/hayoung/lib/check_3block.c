...
void delete_and_create(int array[11][11], int re_val[2][3][5], int Index,int *S)//score
{
	...
	for (i = 0; i < count; i++)
	{
		if(re_val[Index][row_col_dia[i]][0] == 3)
			*S = (*S) + 30;
		else if(re_val[Index][row_col_dia[i]][0] == 4)
			*S = (*S) + 40;
...
void create_block(int array[11][11],int re_val[2][3][5],int *S)
{
	...
	{
		delete_and_create(array, re_val, i, S);
	}
}

int Part_check(int array[11][11], int sx, int sy, int dx, int dy, int re_val[2][3][5],int *S) // re_val setting
{
